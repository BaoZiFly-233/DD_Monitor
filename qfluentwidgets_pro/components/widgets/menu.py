...
from ...common.icon import Action, FluentIconBase, FluentIconEngine, Icon
...
class SubMenuItemWidget(QWidget):
    ...
    def paintEvent(self, e):
        ...
        # draw right arrow
        FIF.CHEVRON_RIGHT.render(
            painter, QRectF(self.width() - 10, self.height() / 2 - 9 / 2, 9, 9)
        )
...
class ShortcutMenuItemDelegate(MenuItemDelegate):
    ...
    def paint(
        ...
        painter.setPen(
            QColor(255, 255, 255, 200) if isDarkTheme() else QColor(0, 0, 0, 153)
        )
...
class MenuActionListWidget(QListWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._maxVisibleItems = (
            -1
        )  # adjust visible items according to the size of screen
    ...
    def adjustSize(self, pos=None, aniType=MenuAnimationType.NONE):
        ...
        if self.maxVisibleItems() > 0:
            size.setHeight(
                min(
                    size.height(),
                    self.maxVisibleItems() * self._itemHeight
                    + m.top()
                    + m.bottom()
                    + 3,
                )
    ...
    def itemsHeight(self):
        ...
        N = (
            self.count()
            if self.maxVisibleItems() < 0
            else min(self.maxVisibleItems(), self.count())
        )
...
class RoundMenu(QMenu):
    ...
    def clear(self):
        ...
        # 修复：addWidget 的项直接进 view（不经过 _actions），QMenu.clear
        # 清不到它们；不清的话菜单重建（如登录菜单刷新）时旧信息项/
        # 分隔线残留，出现"名字上面多一条横线"
        self.view.clear()
    ...
    def addAction(self, action: Union[QAction, Action, str]):
        """add action to menu

        Parameters
        ----------
        action: QAction | Action | str
            menu action（兼容 QMenu 语义：传字符串时自动创建 QAction）

        Returns
        -------
        QAction
            创建的菜单动作（原实现漏了 return，调用方拿 None 无法
            连接 triggered 信号，菜单点击全部失效）
        """
        if isinstance(action, str):
            action = super().addAction(action)
    ...
    def addWidget(self, widget: QWidget, selectable=True, onClick=None):
        ...
        item = self._createActionItem(action)
        # 用 sizeHint 而非 size()：widget 未布局时 size() 为 0，item 塌缩
        # 导致信息项高度/宽度异常
    ...
    def _adjustItemText(self, item: QListWidgetItem, action: QAction):
        ...
        if not self._hasItemIcon():
            ...
            w = (
                60
                + self.view.fontMetrics().boundingRect(item.text()).width()
                + sw
                + space
            )
    ...
    def _longestShortcutWidth(self):
        ...
        return max(
            fm.horizontalAdvance(a.shortcut().toString(QKeySequence.NativeText))
    ...
    def insertAction(
        self, before: Union[QAction, Action], action: Union[QAction, Action]
    ):
    ...
    def addMenu(self, menu):
        ...
        ----------
        menu: RoundMenu | str
            sub round menu（兼容 QMenu 语义：传字符串时自动创建子菜单，
            返回子菜单实例）
        """
    ...
    def mouseMoveEvent(self, e):
        ...
        if (
            self.parentMenu.geometry().contains(pos)
            and not rect.contains(pos)
            and not self.geometry().contains(pos)
        ):
            view.clearSelection()
            self._hideMenu(False)
    ...
    def exec(self, pos, ani=True, aniType=MenuAnimationType.FADE_IN_DROP_DOWN):
        ...
        aniType: MenuAnimationType
            menu animation type（默认 FADE_IN_DROP_DOWN：淡入 + 8px 上滑，
            无 DROP_DOWN 的窗口 mask 裁剪——translucent 窗口上滑动裁剪
            会在 DWM 合成时闪烁黑/白色块）
        """
        if not RoundMenu.animationEnabled:
            ani = False
        ...
        if ani:
            ...
            # 无动画：直接定位显示（不经过动画 manager——QPropertyAnimation
            # 的 start 时序会让菜单先停在默认位置再跳变，造成"菜单
            # 弹不出来/位置错乱"）
            self.move(pos)
...
class MenuAnimationManager(QObject):
    ...
    def _updateMenuViewport(self):
        ...
        修复掉帧：原实现每帧伪造 HoverEnter 事件（强制 WA_UnderMouse），
        触发 item 高亮全量重绘与 ToolTip 检查，配合每帧 setMask 窗口
        裁剪，是展开动画掉帧的主因。hover 高亮由鼠标移动自然触发。
        """
