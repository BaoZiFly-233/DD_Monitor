...
class TabItem(PushButton):
    ...
    def _updateShadowEffect(self):
        ...
        非选中/未启用时 setEnabled(False) 让 Qt 完全跳过 QGraphicsEffect
        的离屏渲染——原实现仅把颜色 alpha 置 0，effect 仍常驻渲染管线，
        标签滑动动画期间每个标签实时重算阴影，是展开动画掉帧的来源。
        """
        can = self._canShowShadow()
        self.shadowEffect.setEnabled(can)
        self.shadowEffect.setColor(QColor(0, 0, 0, 50 * can))
    ...
    def leaveEvent(self, e):
        super().leaveEvent(e)
        if (
            self.closeButtonDisplayMode == TabCloseButtonDisplayMode.ON_HOVER
    ...
    def _drawSelectedBackground(self, painter: QPainter):
        ...
        painter.setBrush(
            ...
            if isDark
            else self.lightSelectedBackgroundColor
        )
...
class TabBar(SingleDirectionScrollArea):
    ...
    def insertTab(
        ...
        item.doubleClicked.connect(
            lambda: self.tabBarDoubleClicked.emit(self.items.index(item))
        )
    ...
    def mousePressEvent(self, e: QMouseEvent):
        ...
        if (
            ...
            or not self.itemLayout.geometry().contains(e.pos())
        ):
            return
    ...
    def mouseMoveEvent(self, e: QMouseEvent):
        ...
        if (
            index == self.count() - 1
            and dx > 0
            and item.geometry().right() >= self.itemLayout.sizeHint().width()
        ):
            return
        ...
        # move the right sibling item to left
            ...
            if (
                item.geometry().right()
                > self.tabItem(siblingIndex).geometry().center().x()
            ):
                self._swapItem(siblingIndex)
