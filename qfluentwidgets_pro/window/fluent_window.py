...
from ..common.icon import FluentIcon, FluentIconBase, drawIcon, toQIcon
...
class FluentTitleBarButton(TitleBarButton):
    ...
    def setMaxState(self, isMax: bool):
        """切换最大化/还原图标（qframelesswindow 的 TitleBar 状态联动）。"""
        self._isMax = bool(isMax)
        self.setIcon(FluentIcon.ZOOM_OUT if isMax else FluentIcon.ZOOM)

    def isMaxState(self) -> bool:
        return self._isMax
...
class FluentTitleBar(TitleBar):
    ...
    def __init__(self, parent):
        ...
        # 移除并隐藏 qframelesswindow 的默认按钮（removeWidget 不移除
        # 父关系，旧按钮会以默认几何残留在标题栏左上角造成重叠）
        for _old in (self.minBtn, self.maxBtn, self.closeBtn):
            self.hBoxLayout.removeWidget(_old)
            _old.hide()
        ...
        self.hBoxLayout.insertWidget(
            0, self.iconLabel, 0, Qt.AlignLeft | Qt.AlignVCenter
        )
        ...
        self.hBoxLayout.insertWidget(
            1, self.titleLabel, 0, Qt.AlignLeft | Qt.AlignVCenter
        )
        self.titleLabel.setObjectName("titleLabel")
        self.window().windowTitleChanged.connect(self.setTitle)

        # 修复：qframelesswindow 的 SvgTitleBarButton 图标固定纯黑，
        # 暗色主题下完全不可见。换用 FluentTitleBarButton（自绘 + 主题色图标）
        self.minBtn = FluentTitleBarButton(FluentIcon.MINIMIZE, self)
        self.maxBtn = FluentTitleBarButton(FluentIcon.ZOOM, self)
        self.closeBtn = FluentTitleBarButton(FluentIcon.CLOSE, self)
        ...
        # 按钮组推到最右（qframelesswindow 的 stretch 移除旧按钮时可能
        # 一并被清掉，显式补一个 stretch 兜底，避免按钮堆在左上角）
