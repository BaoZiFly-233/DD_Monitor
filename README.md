...
| 实时弹幕 | 滚动、顶部、底部三类轨道；透明度、范围、字体、过滤词和同传显示可配置 |
...
全局弹幕设置可以统一修改透明度、显示范围、字体大小、过滤词和进入/礼物信息显示方式。弹幕机是完整工具窗口，关闭后保留状态，再次打开不会重新构造。
...
`python scripts/make_screenshots.py` 会离线构造真实 Qt/OpenGL 窗口并更新三张文档截图，同时验证完整退出路径。脚本使用临时配置，不会覆盖用户数据。
